...
local function add_left_channel(left_ch_name, right_ch_name)
    return string.format(
        ...
        left_ch_name,
        math.max(0, math.min(1, (right - 0.5) * 2)),
        right_ch_name
    )
end
...
local function add_right_channel(left_ch_name, right_ch_name)
    return string.format(
        '%s=%.1f*%s+%.1f*%s',
        right_ch_name,
        math.max(0, math.min(1, (left - 0.5) * 2)),
        left_ch_name,
        math.max(0, math.min(1, right * 2)),
        right_ch_name
    )
end
...
local function update_filter()
    local graph = {}
    local channels =
        opts.forcelayout ~= '' and opts.forcelayout or mp.get_property('audio-params/hr-channels', 'stereo')
    ...
    if
        channels == '3.0' or channels == '3.0(back)' or channels == '3.1' or channels == '5.0' or
    ...
    if
        channels == '2.1' or channels == '3.1' or channels == '4.1' or channels == '5.1' or channels == '5.1(side)' or
            channels == '6.1' or
            channels == '6.1(front)' or
            channels == '7.1' or
            channels == '7.1(wide)' or
            channels == '7.1(side-side)'
     then
        graph[#graph + 1] = 'LFE=LFE'
    end

    if
        channels == '3.0(back)' or channels == '4.0' or channels == '4.1' or channels == '6.0' or
            channels == 'hexagonal' or
            channels == '6.1' or
    ...
    if
        channels == 'quad' or channels == '5.0' or channels == '5.1' or channels == 'hexagonal' or
            channels == '6.1(back)' or
            channels == '7.0' or
            channels == '7.1' or
            channels == '7.1(wide)' or
    ...
    if
        channels == 'quad(side)' or channels == '5.0(side)' or channels == '5.1(side)' or channels == 'hexagonal' or
            channels == '6.0' or
            channels == '6.0(front)' or
            channels == '6.1' or
            channels == '6.1(front)' or
    ...
    if
        channels == '6.0(front)' or channels == '6.1(front)' or channels == '7.0(front)' or channels == '7.1(wide)' or
            channels == '7.1(wide-side)'
     then
        graph[#graph + 1] = add_left_channel('FLC', 'FRC')
        graph[#graph + 1] = add_right_channel('FLC', 'FRC')
    end

    mp.command(string.format('no-osd af add @balance:lavfi=[pan=%s|%s]', channels, table.concat(graph, '|')))

    mp.commandv('show-text', string.format('Audio Balance (pan): Left: %.0f%% Right: %.0f%%', left * 100, right * 100))
end
